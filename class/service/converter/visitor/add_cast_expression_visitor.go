...
import (
	...
	servdecl "bitbucket.org/coontec/javaClass/class/service/converter/model/javasyntax/declaration"
)
...
func (v *AddCastExpressionVisitor) VisitBodyDeclaration( decl *declaration.BodyDeclaration) {
...
func (v *AddCastExpressionVisitor) VisitReturnExpressionStatement( statement *statement.ReturnExpressionStatement) {
	statement.setExpression(updateExpression(returnedType, statement.Expression(), false, true));
}
...
func (v *AddCastExpressionVisitor) VisitThrowStatement( statement *ThrowStatement) {
	if ((exceptionTypes != nil) && (exceptionTypes.size() == 1)) {
		...
		if (exceptionType.isGenericType() && !statement.Expression().Type().equals(exceptionType)) {
			statement.setExpression(addCastExpression(exceptionType, statement.Expression()));
		}
	}
}
...
func (v *AddCastExpressionVisitor)  updateParameters(types _type.IType, expression expression.Expression,  forceCast bool,  unique bool) expression.Expression
if (expressions != nil) {
if (expressions.isList()) {
DefaultList<Type> typeList = types.List();
DefaultList<Expression> expressionList = expressions.List();
...
func (v *AddCastExpressionVisitor)  updateParameter(typ _type.IType, expression expression.Expression,  forceCast bool,  unique bool) expression.Expression {
	...
	if (type == TYPE_BYTE) {
		if (expression.isIntegerConstantExpression()) {
			...
			Expression exp = expression.TrueExpression();
...
func (v *AddCastExpressionVisitor)  updateExpression(typ _type.IType, expression expression.Expression, forceCast bool,  unique bool)  expression.Expression {
	if (expression.isnilExpression()) {
		...
		if (expression.isCastExpression()) {
			Type ceExpressionType = expression.Expression().Type();
...
func (v *AddCastExpressionVisitor)  addCastExpression( typ _type.IType,  expression expression.Expression) expression.Expression{
	if (expression.isCastExpression()) {
		if (type.equals(expression.Expression().Type())) {
...
func (v *AddCastExpressionVisitor) VisitvExpression( expression *expression.vExpression) {}
