...
func (p *ByteCodeParser) parseIINC(statements intmod.IStatements, stack util.IStack[intmod.IExpression],
	...
	if count == 1 {
		...
		expression = expression.NewBinaryOperatorExpression(lineNumber, expression.Type(),
			expression, "+=", expression.NewIntegerConstantExpressionWithAll(lineNumber, expression.Type(), count), 16)
	} else if count < 0 {
		expression = expression.NewBinaryOperatorExpression(lineNumber, expression.Type(),
			expression, "-=", expression.NewIntegerConstantExpressionWithAll(lineNumber, expression.Type(), -count), 16)
	} else {
...
func (p *ByteCodeParser) parseIF(stack util.IStack[intmod.IExpression], lineNumber int,
	...
	if reflect.TypeOf(expression) == reflect.TypeOf(&srvexp.ClassFileCmpExpression{}) {
		...
		stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, cmp.LeftExpression(), operator, cmp.RightExpression(), priority))
		...
		case intmod.FlagBoolean:
			if basicBlock.IsInverseCondition() != ("==" == operator1) {
				...
				stack.Push(expression.NewPreOperatorExpressionWithAll(lineNumber, "!", expression))
			}
		case intmod.FlagFloat:
			stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, expression,
				operator, expression.NewFloatConstantExpressionWithAll(lineNumber, 0), 9))
		case intmod.FlagDouble:
			stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, expression,
				operator, expression.NewDoubleConstantExpressionWithAll(lineNumber, 0), 9))
		case intmod.FlagLong:
			stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, expression,
				operator, expression.NewLongConstantExpressionWithAll(lineNumber, 0), 9))
		default:
			stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, expression,
				operator, expression.NewIntegerConstantExpressionWithAll(lineNumber, pt, 0), 9))
		}
	} else {
		operator := operator2
		if basicBlock.IsInverseCondition() {
		...
		stack.Push(expression.NewBinaryOperatorExpression(lineNumber, model.PtTypeBoolean, expression,
			operator, expression.NewNullExpressionWithAll(lineNumber, expression.Type()), 9))
	}
}
...
func (p *ByteCodeParser) newPreArithmeticOperatorExpression(lineNumber int, operator string,
	...
	return expression.NewPreOperatorExpressionWithAll(lineNumber, operator, expression)
}
...
func (p *ByteCodeParser) newPostArithmeticOperatorExpression(lineNumber int,
	...
	p.reduceIntegerLocalVariableType(expression)
	return expression.NewPostOperatorExpressionWithAll(lineNumber, operator, expression)
}
		...
		if ot.InternalName() != internalName {
			...
			for _, field := range p.bodyDeclaration.FieldDeclarations() {
				...
				if p.memberVisitor.Found() {
					return expression.NewSuperExpressionWithAll(expression.LineNumber(), expression.Type())
				}
			}
		}
	}
